...
algo_desc = algo_descriptor( is_contracted = True, hrr_present = True, aux_index_present = True )
...
class algo_work_array_size_source(contracted_general.contracted_work_array_size_source):
    def serial_out(self,p):
        ...
        The largest the work_array will ever need to be is the largest of,
            2 * (ncc0(lb) * ... ncc0(lX) * n_contraction+ m_max + 1)
        where n_contraction1 is the product of all nX_prim values and b,...X are
        incremented by the end of the VRR sequence,
            2 * ncc0(lb) * ... ncc0(lX) * n_contraction
        where b, ... X are incremented at the end of the VRR sequence, and
        n_contraction is the largest product of nX_prim and nX_cont lengths
        ...
        memreq_expr = self.vrr_memreq_expr(wvrr_group_list[-1],n_contraction1)
